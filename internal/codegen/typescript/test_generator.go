...
func (g *TestGenerator) Generate(i *ir.IR) (*codegen.Output, error) {
	...
	for _, comp := range i.Components {
		if comp.Kind == ir.KindUsecase && comp.Usecase != nil {
			...
			filename := fmt.Sprintf("src/components/usecases/%s.test.ts", sanitizeFilename(comp.ID))
	...
	for _, comp := range i.Components {
		if comp.Kind == ir.KindMiddleware && comp.Middleware != nil {
			...
			filename := fmt.Sprintf("src/components/middlewares/%s.test.ts", sanitizeFilename(comp.ID))
			output.AddFile(filename, []byte(testCode))
		}
	}
	...
	for _, comp := range i.Components {
		if comp.Kind == ir.KindHTTPServer && comp.HTTPServer != nil {
			...
			filename := fmt.Sprintf("src/components/servers/%s.test.ts", sanitizeFilename(comp.ID))
			output.AddFile(filename, []byte(testCode))
		}
	}
...
func (g *TestGenerator) generateUsecaseTest(i *ir.IR, uc *ir.Component) string {
	...
	sb.WriteString("// Generated by Stack Bound - DO NOT EDIT\n")
