...
openboundary transforms YAML specifications into production-ready TypeScript backends. Define your architecture once, generate type-safe code with proper wiring.
...
**Result:** A complete Hono application with authentication middleware, type-safe handlers, and proper dependency injection. You write the business logic, openboundary handles the architecture.
...
```bash
./openboundary compile my-api.yaml
```
...
openboundary doesn't reinvent the wheel. It orchestrates existing tools:
...
**Why this matters:** You use each tool's native format and ecosystem. No leaky abstractions. No "openboundary-flavored" OpenAPI.
...
openboundary generates **structure**, not **implementation**:
...
└── generated/                   # Generated by openboundary
    ├── src/
...
openboundary validates your specifications at multiple levels:
...
```bash
openboundary validate <spec-file>
...
**Example:**
```bash
$ openboundary validate my-api.yaml
✓ Specification is valid

$ openboundary validate my-api.yaml --strict
...
**Example:**
```bash
$ openboundary compile my-api.yaml -o ./backend
...
**Example:**
```bash
$ openboundary init crud -o ./my-project
...
Code generators create files once. openboundary maintains a living relationship between specification and code. Change the spec, regenerate the project. The specification stays current because it's the source of truth.
...
### Can I use openboundary for existing projects?
...
### Does openboundary replace my framework?

No. openboundary generates code using your framework (Hono, Drizzle, better-auth, etc.). You still use those frameworks' APIs and ecosystems. openboundary just wires them together according to your specification.

### What if I need something openboundary doesn't generate?

Write it yourself! openboundary generates structural scaffolding. You write business logic, custom middleware, background jobs, webhooks, or anything else your application needs.
...
openboundary is licensed under the [GNU Affero General Public License v3.0](LICENSE).
...
Star the repo if this looks useful: [github.com/openboundary/openboundary](https://github.com/openboundary/openboundary) ⭐
